...
const passwordModel = require('./models/passwordToken.js');
...
app.post('/user/verifyCode', function(req, res) {
    if (utils.validateVerifyCode(req.body).statusCode == 200) {
        passwordModel.findToken(req.body.token, req.body.email).then(function(passwordResetData) {
            res.status(200);
            var obj = {
                resetPasswordToken: passwordResetData.authToken
            }
            res.status(200);
            res.json(obj);
        }).catch(function(err) {
        ...
        var errorResponse = utils.validateVerifyCode(req.body);
        res.status(errorResponse.statusCode);
        res.json(errorResponse);
    }
});
...
app.post('/user/changePassword', function(req, res) {
    if (utils.validateChangePassword(req.body).statusCode == 200) {
        passwordModel.findAuthToken(req.body.resetPasswordToken).then(function(userData) {
            userModel.updatePassword(req.body).then(function(passwordResetData) {
                var deleteObj = {
                    authToken: req.body.resetPasswordToken,
                    email: req.body.email
                }
                console.log("deleteObj", deleteObj);
                passwordModel.deleteToken(deleteObj).then(function(passwordResetData) {
                    var obj = {
        ...
        var errorResponse = utils.validateChangePassword(req.body);
        res.status(errorResponse.statusCode);
        res.json(errorResponse);
    }
});
